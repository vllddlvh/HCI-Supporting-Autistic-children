<template>
  <div class="min-h-screen md:pl-64 pt-6 font-quicksand">
    <Sidebar />
    ...
    <div class="max-w-5xl mx-auto px-6 pb-20">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 class="text-3xl font-bold text-slate-700 flex items-center gap-3">
            <i class="fas fa-chart-line text-sky-500"></i> Báo cáo học tập
          </h1>
          <p class="text-slate-400 text-sm mt-1">Theo dõi sự tiến bộ theo Cấp độ và Kỹ năng</p>
        </div>
        <div class="kid-card px-4 py-2 text-slate-600 text-sm font-extrabold shadow-none">
           <i class="far fa-calendar-alt mr-2 text-blue-500"></i> {{ currentDate }}
        </div>
      </div>

      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="kid-card p-5 text-center transition-all group hover:-translate-y-0.5">
          <div class="w-12 h-12 mx-auto bg-amber-50 rounded-full flex items-center justify-center mb-3 group-hover:scale-110 transition">
        ...
        <div class="kid-card p-5 text-center transition-all hover:-translate-y-0.5">
          <div class="w-12 h-12 mx-auto bg-sky-50 rounded-full flex items-center justify-center mb-3">
            <i class="fas fa-bullseye text-2xl text-sky-500"></i>
          </div>
          <div class="text-3xl font-extrabold text-sky-600">{{ store.reportData.accuracy || 0 }}%</div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="lg:col-span-1 kid-card p-6 flex flex-col">
          ...
          <div class="flex-1 min-h-[300px] relative flex items-center justify-center">
            ...
            <div v-else class="flex flex-col items-center justify-center text-slate-300 gap-3">
               <div class="kid-card px-6 py-4 shadow-none flex items-center gap-3">
                 <i class="fas fa-spinner fa-spin text-2xl text-sky-500"></i>
                 <span class="text-sm font-extrabold text-slate-600">Đang tải dữ liệu…</span>
               </div>
            </div>
          </div>
        ...
        <div class="lg:col-span-2 kid-card p-6">
            ...
            <div v-for="skill in store.reportData.skillData" :key="skill.code" class="group">
              <div class="flex justify-between items-end mb-2">
                <div class="flex items-center gap-3">
                   <div class="w-10 h-10 rounded-2xl flex items-center justify-center bg-white text-slate-500 border border-slate-100 shadow-sm group-hover:bg-sky-50 group-hover:text-sky-600 transition-colors">
          ...
          <div v-else class="h-64 flex flex-col items-center justify-center text-slate-300">
              <div class="kid-card px-8 py-8 shadow-none text-center">
                <div class="text-5xl mb-3 opacity-80">📒</div>
                <p class="font-extrabold text-slate-700">Chưa có dữ liệu học tập</p>
                <p class="text-sm text-slate-500 font-bold mt-1">Bé học xong, mình quay lại xem nha.</p>
              </div>
          </div>
